...
import { bookingForPeriod, inventoryStats, latestBooking, numeric, quarterlyInventoryStats, reportingPeriods, totalUnits, value, type Project } from "../lib/project-metrics";
...
export default function Home() {
  ...
  const history = (key: "inventory" | "unknownInventory" | "reportedInventory" | "booked" | "unsold" | "pct") => datedStats.map(period => ({
    label: period.label,
  ...
  return <main>
    ...
    <section className="metrics" aria-label="Cumulative figures"><Metric label="Projects" value={filtered.length.toLocaleString("en-IN")} /><Metric label="Inventory" value={stats.inventory.toLocaleString("en-IN")} history={history("inventory")} /><Metric label="Sales data missing" value={stats.unknownInventory.toLocaleString("en-IN")} history={history("unknownInventory")} description="Units · sales status unknown" /><Metric label="Inventory with sales published" value={stats.reportedInventory.toLocaleString("en-IN")} history={history("reportedInventory")} /><Metric label="Booked · latest reported" value={stats.booked.toLocaleString("en-IN")} history={history("booked")} /><Metric label="Unsold inventory (available)" value={stats.unsold === null ? "—" : stats.unsold.toLocaleString("en-IN")} history={history("unsold")} /><Metric label="Booked %" value={`${stats.pct.toFixed(1)}%`} history={history("pct")} /></section>
    <p className="metric-note">Dated figures include only filtered projects approved by that month-end (31 Dec 2025 or 31 Mar 2026). Inventory includes projects without sales reports, using project-details units when report data is missing. Sales data missing counts units with unknown sales status. Booked, Unsold, Inventory with sales published, and Booked % include only projects with published sales data. Mar 2026 falls back to Dec 2025 when March is missing. Unsold sums reported available units only. Headline figures use the latest available report.</p>
...
function Metric({ label, value, history, description }: { label: string; value: string; history?: { label: string; value: string }[]; description?: string }) {
  return <div className="metric"><span>{label}</span><strong>{value}</strong>{description && <small className="metric-description">{description}</small>}
    {history && <dl className="metric-history">{history.map(row => <div key={row.label}><dt>{row.label}</dt><dd>{row.value}</dd></div>)}</dl>}
  </div>;
}
