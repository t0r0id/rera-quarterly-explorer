...
const columns = [
  ["Registration Number", "Registration Number"], ["Builder", "Builder"], ["Promoter Name", "Promoter Name"],
  ["Project Name", "Project Name"], ["District", "District"], ["Taluka", "Taluka"], ["Project Type", "Project Type"],
  ["Approved On", "Approved On"], ["Proposed Completion Date", "Proposed Completion Date"],
  ["Proposed Completion Date At Registration", "Proposed Completion Date At Registration"], ["Project Address", "Project Address"],
  ["Latitude", "Latitude"], ["Longitude", "Longitude"], ["Total Units", "Total Units"], ["Has Quarterly Update", "Has Quarterly Update"],
  ["Units Booked (Q3_FY25-26)", "Booked · Q3 FY25–26"], ["Units Booked (Q4_FY25-26)", "Booked · Q4 FY25–26"],
  ["Units Booked (Q1_FY26-27)", "Booked · Q1 FY26–27"], ["bookedPercent", "Booked %"],
] as const;
...
export default function Home() {
  ...
  const [completionStart, setCompletionStart] = useState(""), [completionEnd, setCompletionEnd] = useState(""), [approvalStart, setApprovalStart] = useState(""), [approvalEnd, setApprovalEnd] = useState(""), [hasUpdate, setHasUpdate] = useState(false), [sort, setSort] = useState<Sort>({ key: "Approved On", direction: "desc" });
  ...
  const clear = () => { setBuilders([]); setPromoters([]); setTalukas([]); setTypes([]); setCompletionStart(""); setCompletionEnd(""); setApprovalStart(""); setApprovalEnd(""); setHasUpdate(false); };
  const sortBy = (key: string) => setSort(s => ({ key, direction: s.key === key && s.direction === "asc" ? "desc" : "asc" }));
  return <main>
    <header className="topbar"><div><p className="eyebrow">Bengaluru Urban · Karnataka RERA</p><h1>Quarterly <em>project</em> explorer</h1></div><button className="filter-toggle" onClick={() => setFiltersOpen(v => !v)}>{filtersOpen ? "Close filters" : "Filters"}<b>{builders.length + promoters.length + talukas.length + types.length + Number(hasUpdate)}</b></button></header>
    ...
    <div className="workspace"><aside className={filtersOpen ? "sidebar open" : "sidebar"}><div className="filter-head"><h2>Refine the view</h2><button onClick={clear}>Clear all</button></div><MultiSelect label="Builder" options={options("Builder")} selected={builders} setSelected={setBuilders} searchable /><MultiSelect label="Promoter" options={options("Promoter Name")} selected={promoters} setSelected={setPromoters} searchable /><MultiSelect label="Taluka" options={options("Taluka")} selected={talukas} setSelected={setTalukas} /><MultiSelect label="Project type" options={options("Project Type")} selected={types} setSelected={setTypes} />
      <fieldset className="filter"><legend>Proposed completion</legend><div className="dates"><input type="date" value={completionStart} onChange={e => setCompletionStart(e.target.value)} /><input type="date" value={completionEnd} onChange={e => setCompletionEnd(e.target.value)} /></div></fieldset><fieldset className="filter"><legend>Approved on</legend><div className="dates"><input type="date" value={approvalStart} onChange={e => setApprovalStart(e.target.value)} /><input type="date" value={approvalEnd} onChange={e => setApprovalEnd(e.target.value)} /></div></fieldset><label className="switch"><input type="checkbox" checked={hasUpdate} onChange={e => setHasUpdate(e.target.checked)} /><span>Has latest quarterly update</span></label></aside>
