...
export default function Home() {
  ...
  const history = (key: "inventory" | "reportedInventory" | "booked" | "unsold" | "pct") => datedStats.map(period => ({
    label: period.label,
  ...
  return <main>
    ...
    <section className="metrics" aria-label="Cumulative figures"><Metric label="Projects" value={filtered.length.toLocaleString("en-IN")} /><Metric label="Inventory" value={stats.inventory.toLocaleString("en-IN")} history={history("inventory")} /><Metric label="Inventory with sales published" value={stats.reportedInventory.toLocaleString("en-IN")} history={history("reportedInventory")} /><Metric label="Booked · latest reported" value={stats.booked.toLocaleString("en-IN")} history={history("booked")} /><Metric label="Unsold inventory (available)" value={stats.unsold === null ? "—" : stats.unsold.toLocaleString("en-IN")} history={history("unsold")} /><Metric label="Booked %" value={`${stats.pct.toFixed(1)}%`} history={history("pct")} /></section>
    <p className="metric-note">Mar 2026 includes Dec 2025 reports when March is missing. Dec 2025 counts only December reports. All metrics use the same selected report per project; inventory falls back to project details if availability is missing. Unsold sums reported available units only. Headline figures use the latest available report.</p>
