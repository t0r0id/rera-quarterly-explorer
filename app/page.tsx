...
export default function Home() {
  ...
  const [completionStart, setCompletionStart] = useState(""), [completionEnd, setCompletionEnd] = useState(""), [approvalStart, setApprovalStart] = useState(""), [approvalEnd, setApprovalEnd] = useState(""), [unitsRange, setUnitsRange] = useState<[number, number] | null>(null), [hasUpdate, setHasUpdate] = useState(true), [sort, setSort] = useState<Sort>({ key: "Total Units", direction: "desc" });
  ...
  const filtered = useMemo(() => projects.filter(p => (!builders.length || builders.includes(p.Builder)) && (!promoters.length || promoters.includes(p["Promoter Name"])) && (!talukas.length || talukas.includes(p.Taluka)) && (!types.length || types.includes(p["Project Type"])) && (!hasUpdate || p["Has Quarterly Update"] === "Yes") && (!unitsRange || (() => { const units = numeric(p["Total Units"]); return units !== null && units >= unitsRange[0] && units <= unitsRange[1]; })()) && dateInRange(p["Proposed Completion Date"] || "", completionStart, completionEnd) && dateInRange(p["Approved On"] || "", approvalStart, approvalEnd)).sort((a,b) => { const av = value(a, sort.key), bv = value(b, sort.key); const missingA = av === null || av === "NA" || av === ""; const missingB = bv === null || bv === "NA" || bv === ""; if (missingA || missingB) return missingA === missingB ? 0 : missingA ? 1 : -1; const an = typeof av === "number" ? av : numeric(av as string); const bn = typeof bv === "number" ? bv : numeric(bv as string); const c = an !== null && bn !== null ? an - bn : String(av).localeCompare(String(bv)); return sort.direction === "asc" ? c : -c; }), [projects, builders, promoters, talukas, types, completionStart, completionEnd, approvalStart, approvalEnd, unitsRange, hasUpdate, sort]);
  ...
  const clear = () => { setBuilders([]); setPromoters([]); setTalukas([]); setTypes([]); setCompletionStart(""); setCompletionEnd(""); setApprovalStart(""); setApprovalEnd(""); setUnitsRange(null); setHasUpdate(true); };
  ...
  return <main>
    ...
    <section className="metrics" aria-label="Cumulative figures"><Metric label="Projects" value={filtered.length.toLocaleString("en-IN")} /><Metric label="Inventory" value={stats.inventory.toLocaleString("en-IN")} /><Metric label="Inventory with sales published" value={stats.reportedInventory.toLocaleString("en-IN")} /><Metric label="Booked · latest reported" value={stats.booked.toLocaleString("en-IN")} /><Metric label="Booked %" value={`${stats.pct.toFixed(1)}%`} /></section>
    <div className="workspace"><aside className={filtersOpen ? "sidebar open" : "sidebar"}><div className="filter-head"><h2>Refine the view</h2><button onClick={clear}>Reset</button></div><fieldset className="filter"><legend>Approved on date</legend><div className="dates"><input type="date" value={approvalStart} onChange={e => setApprovalStart(e.target.value)} /><input type="date" value={approvalEnd} onChange={e => setApprovalEnd(e.target.value)} /></div></fieldset><fieldset className="filter"><legend>Completion on date</legend><div className="dates"><input type="date" value={completionStart} onChange={e => setCompletionStart(e.target.value)} /><input type="date" value={completionEnd} onChange={e => setCompletionEnd(e.target.value)} /></div></fieldset>{unitsBounds && <fieldset className="filter"><legend>Total units<span>{(unitsRange || unitsBounds).map(n => n.toLocaleString("en-IN")).join(" – ")}</span></legend><div className="range"><input aria-label="Minimum total units" type="range" min={unitsBounds[0]} max={unitsBounds[1]} value={(unitsRange || unitsBounds)[0]} onChange={e => { const next = Number(e.target.value); setUnitsRange([Math.min(next, (unitsRange || unitsBounds)[1]), (unitsRange || unitsBounds)[1]]); }} /><input aria-label="Maximum total units" type="range" min={unitsBounds[0]} max={unitsBounds[1]} value={(unitsRange || unitsBounds)[1]} onChange={e => { const next = Number(e.target.value); setUnitsRange([(unitsRange || unitsBounds)[0], Math.max(next, (unitsRange || unitsBounds)[0])]); }} /></div></fieldset>}<MultiSelect label="Taluk" options={options("Taluka")} selected={talukas} setSelected={setTalukas} /><MultiSelect label="Builder" options={options("Builder")} selected={builders} setSelected={setBuilders} searchable /><MultiSelect label="Promoter" options={options("Promoter Name")} selected={promoters} setSelected={setPromoters} searchable /><MultiSelect label="Project type" options={options("Project Type")} selected={types} setSelected={setTypes} />
      ...
      <section className="table-panel"><div className="table-meta"><span>{filtered.length.toLocaleString("en-IN")} matching projects</span><span>Drag headings to reorder · Click to sort · <b>*</b> Q1 unavailable, latest prior quarter shown</span></div><div className="table-wrap"><table><thead><tr>{columnOrder.map(([key,label]) => <th key={key} draggable onDragStart={() => setDraggedColumn(key)} onDragOver={event => event.preventDefault()} onDrop={() => moveColumn(key)} onDragEnd={() => setDraggedColumn(null)} className={draggedColumn === key ? "dragging" : ""}><button onClick={() => sortBy(key)}><span className="drag-handle" aria-hidden="true">⠿</span>{label}<i className={sort.key === key ? "active" : ""}>{sort.key === key ? (sort.direction === "asc" ? "↑" : "↓") : "↕"}</i></button></th>)}</tr></thead><tbody>{filtered.map(p => <tr key={p["Registration Number"]}>{columnOrder.map(([key]) => { const v = value(p,key); const mapUrl = googleMapsUrl(p); const booking = latestBooking(p); const hasFallback = key === "Units Booked (Q1_FY26-27)" || key === "bookedPercent" ? booking?.fallback : false; const fallbackTitle = hasFallback ? `${booking?.quarter} used because Q1 FY26–27 is unavailable` : String(v ?? "—"); return <td key={key} title={fallbackTitle}>{key === "Project Address" ? <>{v || "—"}{mapUrl && <a className="map-link" href={mapUrl} target="_blank" rel="noreferrer">Map ↗</a>}</> : key === "Has Quarterly Update" ? <span className={v === "Yes" ? "tag yes" : "tag"}>{v === "Yes" ? "Yes" : "No"}</span> : key === "bookedPercent" ? v === null ? "—" : <span className="fill" style={{ "--fill": `${Math.min(100, v as number)}%` } as React.CSSProperties}><b>{(v as number).toFixed(1)}%{hasFallback && <sup>*</sup>}</b></span> : key === "Units Booked (Q1_FY26-27)" ? <>{v || "—"}{hasFallback && <sup>*</sup>}</> : v || "—"}</td>; })}</tr>)}</tbody></table>{!projects.length && <p className="loading">Loading RERA project data…</p>}{projects.length > 0 && !filtered.length && <p className="loading">No projects match these filters.</p>}</div></section></div>
