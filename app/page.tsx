...
export default function Home() {
  ...
  const filtered = useMemo(() => projects.filter(p => (!builders.length || builders.includes(p.Builder)) && (!promoters.length || promoters.includes(p["Promoter Name"])) && (!talukas.length || talukas.includes(p.Taluka)) && (!types.length || types.includes(p["Project Type"])) && (!hasUpdate || p["Has Quarterly Update"] === "Yes") && dateInRange(p["Proposed Completion Date"] || "", completionStart, completionEnd) && dateInRange(p["Approved On"] || "", approvalStart, approvalEnd)).sort((a,b) => { const av = value(a, sort.key), bv = value(b, sort.key); const missingA = av === null || av === "NA" || av === ""; const missingB = bv === null || bv === "NA" || bv === ""; if (missingA || missingB) return missingA === missingB ? 0 : missingA ? 1 : -1; const an = typeof av === "number" ? av : numeric(av as string); const bn = typeof bv === "number" ? bv : numeric(bv as string); const c = an !== null && bn !== null ? an - bn : String(av).localeCompare(String(bv)); return sort.direction === "asc" ? c : -c; }), [projects, builders, promoters, talukas, types, completionStart, completionEnd, approvalStart, approvalEnd, hasUpdate, sort]);
