...
export default function Home() {
  ...
  return <main>
    ...
    <section className="metrics" aria-label="Cumulative figures"><Metric label="Projects" value={filtered.length.toLocaleString("en-IN")} history={history("projects")} /><Metric label="Inventory" value={stats.inventory.toLocaleString("en-IN")} history={history("inventory")} /><Metric status="unknown" label="Sales data no published" value={stats.unknownInventory.toLocaleString("en-IN")} history={history("unknownInventory")} percentage={shares.unknownInventory} description="Units · sales status unknown" /><Metric status="booked" label="Booked · latest reported" value={stats.booked.toLocaleString("en-IN")} history={history("booked")} percentage={shares.booked} /><Metric status="unsold" label="Unsold inventory (available)" value={(stats.unsold ?? 0).toLocaleString("en-IN")} history={history("unsold")} percentage={shares.unsold} /></section>
