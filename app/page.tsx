...
import { inventoryStats, latestBooking, numeric, quarterlyInventoryStats, reportingPeriods, totalUnits, value, type Project } from "../lib/project-metrics";
...
export default function Home() {
  ...
  const datedStats = useMemo(() => reportingPeriods.map(period => ({
    label: period.label,
    ...quarterlyInventoryStats(filtered, period.suffix),
  })), [filtered]);
  const history = (key: "inventory" | "reportedInventory" | "booked" | "pct") => datedStats.map(period => ({
    label: period.label,
  ...
  return <main>
    ...
    <section className="metrics" aria-label="Cumulative figures"><Metric label="Projects" value={filtered.length.toLocaleString("en-IN")} /><Metric label="Inventory" value={stats.inventory.toLocaleString("en-IN")} history={history("inventory")} /><Metric label="Inventory with sales published" value={stats.reportedInventory.toLocaleString("en-IN")} history={history("reportedInventory")} /><Metric label="Booked · latest reported" value={stats.booked.toLocaleString("en-IN")} history={history("booked")} /><Metric label="Booked %" value={`${stats.pct.toFixed(1)}%`} history={history("pct")} /></section>
    <p className="metric-note">Dated rows count only projects with sales published for that quarter, for both booked units and inventory. Inventory uses that quarter’s booked + available, or project-details units when availability is missing. Headline figures use the latest available report.</p>
...
const ProjectRow = memo(function ProjectRow({ project: p, columnOrder }: { project: Project; columnOrder: Column[] }) {
  ...
  return <tr>{columnOrder.map(([key]) => {
    ...
    const title = key === "Total Units"
      ...
      : hasFallback ? `${booking?.quarter} used because Jun 2026 is unavailable` : String(v ?? "—");
    return <td key={key} title={title}>{
      ...
      key === "Units Booked (Q1_FY26-27)" ? <>{v ?? "—"}{hasFallback && <sup>*</sup>}</> : v ?? "—"
    }</td>;
  })}</tr>;
});

function Metric({ label, value, history }: { label: string; value: string; history?: { label: string; value: string }[] }) {
  return <div className="metric"><span>{label}</span><strong>{value}</strong>
