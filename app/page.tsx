...
export default function Home() {
  ...
  const stats = useMemo(() => { const total = filtered.reduce((s,p) => s + (numeric(p["Total Units"]) || 0), 0); const booked = filtered.reduce((s,p) => s + (latestBooking(p)?.booked || 0), 0); return { total, booked, pct: total ? booked / total * 100 : 0 }; }, [filtered]);
