...
function Metric({ label, value, history, description, percentage }: { label: string; value: string; history?: { label: string; value: string; percentage?: number | null }[]; description?: string; percentage?: number | null }) {
  return <div className="metric"><span>{label}</span><strong>{value}{percentage !== undefined && <small className="metric-percentage">({percentage === null ? "—" : `${percentage.toFixed(1)}%`})</small>}</strong>{description && <small className="metric-description">{description}</small>}
